...
*/
    ...
    public class Track : MonoBehaviour
    {
        ...
        protected Mesh TryToFindGeneratedMesh()
        {
            ...
            if (FindObjectsOfType<MeshFilter>().Any(x => x != mf && x.sharedMesh == currentMesh))
                return null;
        ...
        /** Updates the physics system with our list of ignored colliders
        *after fiddling with our collider, deactivation, etc. */
        protected void RefreshIgnoredColliders()
        {
            ...
            if (NextTrack)
            {
                foreach (Collider other in NextTrack.ignoredColliders)
                {
                    __ignoreCollisions(other, true);
            ...
            if (PrevTrack)
            {
                foreach (Collider other in PrevTrack.ignoredColliders)
                {
                    __ignoreCollisions(other, true);
                }
            }
        ...
        protected void __ignoreCollisions(Collider other, bool ignore)
        {
            ...
            MeshCollider oMC = other as MeshCollider;
        ...
        protected void _IgnoreCollisionIfAble(Collider a, Collider b, bool ignore)
        {
            ...
            MeshCollider aMC = a as MeshCollider;
            if (aMC && (!aMC.sharedMesh || aMC.sharedMesh.vertexCount == 0)) return;

            MeshCollider bMC = a as MeshCollider;
